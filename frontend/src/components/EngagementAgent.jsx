...
export default function EngagementAgent({ businessId }) {
  ...
  useEffect(() => {
    ...
    async function fetchAgentData() {
      try {
        ...
        // Replace with your backend URL - remove trailing slash if present
        const backendUrl = (
          import.meta.env.VITE_BACKEND_URL || "http://localhost:3000"
        ).replace(/\/$/, "");
        const response = await fetch(
          `${backendUrl}/api/engagement-agent/${businessId}`
        );
  ...
  function createDummyCampaign() {
    ...
    const dummyCampaign = {
      ...
      priority: 50,
      message:
        "Hi! We have a special offer just for you. Get 15% off your next visit!",
            ...
            >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center space-x-2">
                      <p className="text-sm font-medium text-gray-900">
                        {rec.customerName || "General Recommendation"}
                      </p>
                      <span
                  ...
                  <button
                    onClick={() => {
                      ...
                      const fallbackMessage =
                        rec.message ||
                        `Hi ${rec.customerName || ""}, you've unlocked ${
                          rec.discountPercentage
                        }% off your next ${
                          rec.serviceName || "visit"
              ...
              {rec.discountPercentage && !rec.customerId && (
                <div className="mt-2 text-xs text-gray-500 italic">
                  General recommendation - create a campaign manually to send to
                  multiple customers
                </div>
              )}
            </div>
            ...
            <div className="flex items-center space-x-2 mb-4 pb-4 border-b border-gray-200">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900">
                  Business Owner Authorization Required
                </h3>
                <p className="text-xs text-gray-500 mt-0.5">
                  Confirm your identity before sending campaign
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4 mb-6">
                ...
                <div className="space-y-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-blue-900">
                      {selectedRecommendation.serviceName || "N/A"}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  ...
                  before sending. For the demo, this will be sent to your
                ...
                <label className="flex items-start space-x-3 cursor-pointer">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-yellow-900">
                      I confirm that I am the business owner/authorized
                      representative
                    </p>
                    <p className="text-xs text-yellow-700 mt-1">
                      By checking this box, I authorize sending this discount
                      campaign to{" "}
                      <strong>{selectedRecommendation.customerName}</strong> via
                      your configured Twilio messaging channel.
                    </p>
                    <p className="text-xs text-yellow-800 mt-2 font-medium">
                      ⚠️ This action cannot be undone. The customer will receive
                      the message immediately upon confirmation.
                    </p>
                  </div>
                </label>
              </div>
              ...
              <button
                onClick={() => {
                  ...
                  const permissionCheckbox =
                    document.getElementById("owner-permission");
              ...
              <button
                onClick={async (e) => {
                  ...
                  if (!permissionCheckbox?.checked) {
                    alert(
                      "⚠️ Please confirm that you are the business owner/authorized representative by checking the box above."
                    );
                    return;
                  }
                  ...
                  if (!businessId) {
                    alert(
                      "❌ Business ID is missing. Please refresh the page."
                    );
                    return;
                  }
                  ...
                  try {
                    ...
                    const discountCode = `ENG${Math.floor(Math.random() * 10000)
                      .toString()
                      .padStart(4, "0")}`;

                    const response = await fetch(
                      `${backendUrl}/api/campaigns/send`,
                      {
                        method: "POST",
                        headers: {
                        ...
                        body: JSON.stringify({
                          ...
                          campaignName: `Engagement Agent: ${
                            selectedRecommendation.serviceName ||
                            "Special Offer"
                          }`,
                          campaignType: "percentage",
                          campaignValue:
                            selectedRecommendation.discountPercentage,
                          discountCode,
                          customerIds: [
                            selectedRecommendation.customerId,
                          ].filter((id) => id != null),
                    ...
                    try {
                      ...
                      throw new Error(
                        `Backend error: ${text || response.statusText}`
                      );
                    }
                    ...
                    if (!response.ok) {
                      ...
                      if (
                        result?.demo ||
                        result?.message?.includes("Demo mode")
                      ) {
                        alert(
                    ...
                    const successMessage = result.demo
                      ? `✅ Demo Mode: Campaign would be sent!\n\nDiscount code: ${discountCode}\n${
                          result.message ||
                          `Would send to ${result.sent || 1} customer(s)`
                        }`
                      : `✅ Campaign sent for ${
                          selectedRecommendation.customerName || "this customer"
                        }!\n\nDiscount code: ${discountCode}\nSent to ${
                          result.sent || 0
                        } customer(s) via your Twilio demo WhatsApp number.`;
