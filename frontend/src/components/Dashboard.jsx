...
const Dashboard = () => {
  ...
  const StatCard = ({ icon: Icon, title, value, trend, delay, gradient }) => (
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <h3 className="text-white text-opacity-90 text-sm font-semibold mb-2 uppercase tracking-wide">
          {title}
        </h3>
  ...
  const FeatureCard = ({ icon: Icon, title, description, gradient, delay }) => (
    ...
    >
      ...
      <div className="relative z-10">
        <div
          className={`inline-flex p-4 rounded-2xl bg-gradient-to-br ${gradient} mb-5 transform group-hover:scale-110 group-hover:rotate-6 transition-all duration-300 shadow-lg`}
        >
          <Icon size={32} className="text-white" />
        </div>
        <h3 className="text-2xl font-bold text-gray-800 mb-3 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-purple-600 group-hover:to-blue-600 transition-all duration-300">
          {title}
        </h3>
        ...
        <div className="mt-4 flex items-center text-purple-600 opacity-0 group-hover:opacity-100 transform translate-x-0 group-hover:translate-x-2 transition-all duration-300">
          <span className="font-semibold mr-2">{t("dashboard.learnMore")}</span>
          <svg
            className="w-5 h-5"
            fill="none"
            stroke="currentColor"
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-purple-100 to-pink-100 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 py-5">
          <div className="flex items-center justify-between">
            ...
            <button className="px-8 py-4 bg-gradient-to-r from-purple-600 via-blue-600 to-pink-600 text-white rounded-xl font-bold text-lg shadow-xl hover:shadow-2xl transform hover:scale-110 hover:-rotate-2 transition-all duration-300 relative overflow-hidden group">
              <span className="relative z-10">{t("dashboard.getStarted")}</span>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-10">
        {/* Hero Section */}
        <div
          className={`relative overflow-hidden bg-gradient-to-r from-purple-600 via-blue-600 to-pink-600 rounded-3xl shadow-2xl p-12 mb-10 text-white transform transition-all duration-1000 ${
            animate
              ? "scale-100 opacity-100 rotate-0"
              : "scale-95 opacity-0 rotate-2"
          }`}
        >
          {/* Animated sparkles */}
          ...
          <div className="relative z-10">
            <h2 className="text-5xl font-black mb-5 drop-shadow-lg">
            ...
            <div className="flex flex-wrap gap-4">
              {[
                {
                  icon: PhoneIcon,
                  text: t("dashboard.automatedCalls"),
                  bg: "from-blue-500 to-cyan-400",
                },
                {
                  icon: CalendarIcon,
                  text: t("dashboard.smartScheduling"),
                  bg: "from-purple-500 to-pink-400",
                },
                {
                  icon: DatabaseIcon,
                  text: t("dashboard.crmIntegration"),
                  bg: "from-green-500 to-teal-400",
                },
              ].map((item, i) => (
                <div
                  key={i}
                  className={`flex items-center space-x-3 bg-gradient-to-r ${item.bg} rounded-xl px-5 py-3 shadow-lg transform hover:scale-110 transition-transform duration-300 cursor-pointer`}
                >
                  <item.icon size={24} />
        ...
        <div className="mb-10">
          <h2 className="text-4xl font-black text-gray-800 mb-8 text-center">
            <span className="bg-gradient-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent">
              {t("dashboard.powerfulFeatures")}
            </span>
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <FeatureCard
              icon={PhoneIcon}
              title={t("features.aiCallAssistant.title")}
              description={t("features.aiCallAssistant.description")}
            ...
            <FeatureCard
              icon={DatabaseIcon}
              title={t("features.crmIntegration.title")}
              description={t("features.crmIntegration.description")}
              gradient="from-green-500 to-teal-500"
              delay={300}
            />
            <FeatureCard
              icon={MessageIcon}
              title={t("features.smartFollowups.title")}
              description={t("features.smartFollowups.description")}
              gradient="from-orange-500 to-red-500"
              delay={400}
            />
            <FeatureCard
              icon={DollarIcon}
              title={t("features.dynamicDiscounts.title")}
              description={t("features.dynamicDiscounts.description")}
              gradient="from-yellow-500 to-orange-500"
              delay={500}
            />
            <FeatureCard
              icon={TargetIcon}
              title={t("features.businessInsights.title")}
              description={t("features.businessInsights.description")}
        ...
        {/* CTA Section */}
        <div
          className={`relative overflow-hidden bg-gradient-to-r from-pink-600 via-purple-600 to-blue-600 rounded-3xl shadow-2xl p-12 text-white text-center transform transition-all duration-1000 ${
        ...
        >
          ...
          <div className="relative z-10">
            <h2 className="text-5xl font-black mb-5 drop-shadow-lg">
              {t("dashboard.readyTransform")}
            </h2>
            <p className="text-2xl font-medium mb-8">
              {t("dashboard.joinHundreds")}
            </p>
