...
export default function CrossReferenceAgent({ businessId }) {
  ...
  useEffect(() => {
    ...
    if (!businessId) {
      setAgentData({
        ...
        summary:
          "Found 3 similar businesses. Top match: Beauty Salon Pro (Beauty). 2 share your industry. 1 active partnership.",
    ...
    async function fetchAgentData() {
      try {
        ...
        const [agentResponse, partnershipsResponse] = await Promise.all([
          fetch(`${backendUrl}/api/cross-reference/${businessId}`).catch(
            () => ({ ok: false })
          ),
          fetch(
            `${backendUrl}/api/cross-reference/${businessId}/partnerships`
          ).catch(() => ({ ok: false })),
        ]);
        ...
        if (!agentResponse.ok) {
          ...
          setAgentData({
            ...
            summary:
              "Demo mode: Found 2 similar businesses for cross-promotion.",
            currentBusiness: {
              id: businessId,
              name: "Your Business",
              industry: "Beauty",
  ...
  async function handleCreatePartnership(otherBusinessId) {
    try {
      ...
      const partnerName =
        agentData?.recommendations?.find((r) => r.id === otherBusinessId)
          ?.name || "partner";
      ...
      const response = await fetch(
        `${backendUrl}/api/cross-reference/${businessId || 1}/partnership`,
        {
          method: "POST",
          headers: {
      ...
      try {
        ...
        if (!businessId) {
          alert(
            `✅ Demo: Partnership would be created with ${partnerName}! Your promotions would be shared with their customers and vice versa.`
          );
          ...
          if (agentData) {
            ...
            setPartnerships([
              ...partnerships,
              {
                ...
                partnerBusiness: {
                  name: partnerName,
                  industry: agentData.recommendations.find(
                    (r) => r.id === otherBusinessId
                  )?.industry,
                },
              },
            ]);
      ...
      alert(
        `✅ Partnership created! Your promotions will now be shared with ${partnerName} customers and vice versa.`
      );
    } catch (error) {
  ...
  async function handleDeactivatePartnership(partnershipId, partnerName) {
    if (
      !confirm(
        `Are you sure you want to deactivate the partnership with ${partnerName}?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        `${backendUrl}/api/cross-reference/${businessId}/partnership/${partnershipId}/deactivate`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                Find similar businesses for cross-promotion
              </p>
            </div>
          </div>
        </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-600">
                      {partnership.partnerBusiness?.industry ||
                        "Unknown Industry"}
                    </p>
                  </div>
                </div>
        ...
        <div className="space-y-3">
          {recommendations.length > 0 ? (
            recommendations.map((rec) => (
              <div
                key={rec.id}
                className={`border-2 rounded-lg p-5 ${getScoreColor(
                  rec.score
                    ...
                    <div className="flex-1">
                      {rec.isPartnered ? (
                        <div className="flex items-start space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              Your campaigns are automatically shared with their
                              customers
                            </p>
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            Start cross-promoting with {rec.name} and reach new
                            customers together
                          </p>
                        </div>
                      )}
                    ...
                    <button
                      onClick={() => {
                        if (!rec.isPartnered) {
                          if (
                            confirm(
                              `Approve partnership with ${rec.name}? Your promotions will be shared with their customers and vice versa.`
                            )
                          ) {
                      ...
                      disabled={
                        creatingPartnership === rec.id || rec.isPartnered
                      }
                      type="button"
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-xs mt-1">
                Try adding more businesses to your database
              </p>
            </div>
          )}
        </div>
        ...
        <div className="flex items-start space-x-2">
          ...
          <div>
            ...
            <p className="text-xs text-blue-700">
              When you opt into a partnership, your promotional campaigns will
              be shared with the partner business's customers, and their
              campaigns will be shared with yours. This helps both businesses
              reach new audiences with similar interests.
            </p>
          </div>
        </div>
      </div>
